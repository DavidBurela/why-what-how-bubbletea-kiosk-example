...
- **Rich report (HTML)** — viewable locally when the repo is cloned, or as a CI artifact. Provides detailed pass/fail information, durations, and error messages.
- **Summary (markdown)** — renderable on platforms like GitHub without cloning. Maps scenario IDs to pass/fail status with counts and a timestamp of the last run.
...
- Individual scenario files can link to the verification summary for their scenario status.
